...
ll minAbsDiff(ll n, vector<ll> &fList)
{
    ll minAbsVal = LLONG_MAX;

    for(ll i=fList.begin(); i!=fList.end(); i++)
    {
        minAbsVal = min(minAbsVal, abs((i*) - n));
    }
...
ll solve() 
{
    ...
    {
        fList.push_back(f[i] - 'a');
    }

    for(int i=0; i<s.size(); i++)
    {
        ans += minAbsDiff(s[i] - 'a', fList);
    }
